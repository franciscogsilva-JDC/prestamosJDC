        ...
        <div class="row">	
			<div class="col s12 m12 l12">	
                <div class="card-panel card-panel-search-fgs">	
					...
					{!! Form::open(['route' => 'publications.index', 'method' => 'GET'], ['class' => 's12 m12 l12']) !!}
						<div class="row row-search-fgs">
							<div class="file-field input-field col s12 m12 l9 input-search-fgs">
								<div class="file-path-wrapper path-wrapper-fgs center-text">
									...
									<label class="label-search-fgs" for="icon_prefix">Buscar publicación, titulo</label>
								</div>
							</div>
						...
						<div class="section section-form">
                        	<div class="row" id="panel-filters">
								<div class="input-field col s12 m6 l6">
									...
									<label for="publication_type_id">Filtrar por Tipo de publicación</label>
								</div>
								<div class="input-field col s12 m6 l6">
									<i class="material-icons prefix">visibility_off</i>
									<select id="publication_status_id" name="publication_status_id">
										<option value="" disabled selected>Selecciona un Estado de publicación</option>
										@foreach($publication_statuses as $type)
											<option value="{{ $type->id }}">{{ $type->name }}</option>
										@endforeach
									</select>
									<label for="publication_status_id">Filtrar por Estado de publicación</label>
								</div>								
                        	</div>
                        </div>
	                {!! Form::close() !!}
				...
				<div class="card-panel card-panel-table-factories">
					<div class="card-content">
						{!! Form::open(['route' => 'publications.multi_destroy', 'method' => 'POST'], ['class' => '']) !!}
						<div class="row row-delete-all">							
							...
							{!! Form::submit('Borrar', ['class' => 'btn red darken-3 right', 'onclick' => 'return confirm("¿Desea borrar las publicaciones seleccionadas?")', 'id' => 'multiDeleteAction', 'style' => 'display: none;']) !!}	
						</div>
			     		<table class="highlight striped">
							...
							<tbody>
								...
								<tr>									
									...
									@if($publication->publication_status->id == 1)
										<td id="td-sector" class="center-align">
											<span class="badge green badge-status-factory center-text">{{ $publication->publication_status->name }}</span>
										</td>
									@elseif($publication->publication_status->id == 2)
										<td id="td-sector" class="center-align">
											<span class="badge blue badge-status-factory center-text">{{ $publication->publication_status->name }}</span>
										</td>
									@endif
									<td id="td-logo">{{ $publication->author_other }}</td>
									<td class="td-fgs center-align">
	                					...
	                					{!! Form::close() !!}
										<a href="{{ route('publications.destroy', $publication->id) }}" onclick="return confirm('¿Desea borrar la publicación?')" class="btn btn-fgs btn-fgs-delete red darken-3"><i class="material-icons">delete</i></a>
										<a href="{{ route('publications.edit', $publication->id) }}" class="btn btn-raised btn-primary btn-fgs btn-fgs-edit"><i class="material-icons">create</i></a>
									</td>
								</tr>
								@endforeach
							</tbody>
